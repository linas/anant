...
/** Bernoulli number B_n given as a fixed-point number. */
void q_bernoulli (mpq_t bern, int n);

/** Compute and return the "exact" result for the zeta function for 
 * any value of even n. Computed to `prec` decimal places.
 * Zeta at the even values of n can be given exactly as a product of
 * a Bernoulli number and factors of pi.
 * This proceeds by computing the Bernoulli number first.
 */
void fp_zeta_even (mpf_t zeta, unsigned int n, int prec);

/** fp_zeta
 * Floating-point-valued Riemann zeta for positive integer arguments.
 * Return value is placed in the arg "zeta".
 *
 * Calculations performed to `prec` decimal digits.
 */
